...
const createReminderReactionMessageTable = (database) => {
  database.run(
    `CREATE TABLE IF NOT EXISTS ReminderReactionMessage(uuid TEXT NOT NULL PRIMARY KEY, created_at DATE NOT NULL, requested_by TEXT NOT NULL, channel_id TEXT NOT NULL, guild_id TEXT NOT NULL, reminder_id TEXT NOT NULL, no_of_reactions INTEGER NOT NULL)`
  );
};
/**
...
const insertNewReminderReactionMessage = (message, user, reminder) => {
  ...
  database.serialize(() => {
    database.run(
      'INSERT INTO ReminderReactionMessage(uuid, created_at, requested_by, channel_id, guild_id, reminder_id, no_of_reactions) VALUES ($uuid, $created_at, $requested_by, $channel_id, $guild_id, $reminder_id, $no_of_reactions)',
      {
        $uuid: message.id,
    ...
    database.run(
      `UPDATE Reminder SET reaction_message_id = "${message.id}" WHERE uuid = "${reminder.uuid}"`,
      (error) => {
        if (error) {
          console.log(error);
        }
  ...
  database.serialize(() => {
    ...
    guilds.forEach((guild) => {
      guild.channels.cache.forEach((channel) => {
        database.each(
          `SELECT * FROM ReminderReactionMessage WHERE channel_id = ${channel.id}`,
          async (error, detail) => {
            if (error) {
              console.log(error);
            }
            if (detail) {
              try {
                ...
                database.serialize(() => {
                  database.each(
                    `SELECT * FROM Reminder WHERE reaction_message_id = ${detail.uuid}`,
                    (error, reminder) => {
                      database.run(
                        `UPDATE Reminder SET reaction_message_id = ${null} WHERE uuid = "${
                          reminder.uuid
                        }"`
                      );
                    }
                    ...
                    (error, reminder) => {
                      if (reminder) {
                        database.get(
                          ...
                          (error, role) => {
                            database.each(
                              `SELECT * FROM ReminderUser WHERE reminder_reaction_message_id = "${detail.uuid}"`,
                              async (error, user) => {
                                const memberToRemove = await guild.members.fetch(user.user_id);
                                memberToRemove.roles.remove(role.role_id);
                                database.run(
                                  `DELETE FROM ReminderUser WHERE uuid = "${user.uuid}"`
                                );
                              }
                            );
                          }
...
const updateReminderReactionMessage = (reaction) => {
  let database = new sqlite.Database('./database/yagi.db', sqlite.OPEN_READWRITE);
  database.get(
    `SELECT * FROM ReminderReactionMessage WHERE uuid = "${reaction.message.id}"`,
    (error, detail) => {
      if (error) {
        console.log(error);
      }
      if (detail && !reaction.me && reaction.emoji.identifier === '%F0%9F%90%90') {
        ...
        database.run(
          `UPDATE ReminderReactionMessage SET no_of_reactions = ${newCount} WHERE uuid = ${detail.uuid}`,
          (error) => {
            if (error) {
              console.log(error);
            }
...
const checkIfReminderReactionMessage = (message, oldMessage) => {
  let database = new sqlite.Database('./database/yagi.db', sqlite.OPEN_READWRITE);
  database.get(
    `SELECT * FROM ReminderReactionMessage WHERE uuid = "${message.id}"`,
    (error, reactionMessage) => {
      if (
        reactionMessage &&
        message.content === '' &&
        oldMessage.embeds.length === 1 &&
        message.embeds.length === 0 &&
        (message.author.id === '582202266828668998' || message.author.id === '518196430104428579')
      ) {
        message.delete();
      }
    }
      ...
      (error, reactionMessage) => {
        if (reactionMessage) {
          database.serialize(() => {
            //Get reminder
            database.get(
              `SELECT * FROM Reminder WHERE reaction_message_id = ${message.id}`,
              (error, reminder) => {
                //Get reminder role
                database.get(
                  `SELECT * FROM Role WHERE uuid = "${reminder.role_uuid}"`,
                  (error, role) => {
                    ...
                    database.each(
                      `SELECT * FROM ReminderUser WHERE reminder_reaction_message_id = "${message.id}"`,
                      async (error, user) => {
                        if (role) {
             ...
             */
            database.get(
              `SELECT * FROM Reminder WHERE reaction_message_id = "${
                message.id
              }" AND enabled = ${true}`,
              async (error, reminder) => {
                if (reminder) {
                  ...
                  database.run(
                    `UPDATE Reminder SET timer = ${null}, enabled = ${false} WHERE uuid = "${
                      reminder.uuid
                    }"`
                  );
                }
  ...
  database.get(
    ...
    async (error, reactionMessage) => {
      if (reactionMessage) {
        ...
        database.run(
          `UPDATE Reminder SET reaction_message_id = "${reactionMessage.uuid}" WHERE uuid = "${reminder.uuid}"`,
          (err) => {
        ...
        const messageDetail = await message.channel.send({ embeds: [embed] });
