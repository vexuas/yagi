...
const createReminderTable = (database) => {
  database.run(
    'CREATE TABLE IF NOT EXISTS Reminder(uuid TEXT NOT NULL PRIMARY KEY, created_at DATE NOT NULL, enabled BOOLEAN NOT NULL, enabled_by TEXT, enabled_at DATE, disabled_by TEXT, disabled_at DATE, type TEXT NOT NULL, role_uuid TEXT, channel_id TEXT, guild_id TEXT, reaction_message_id TEXT, timer BLOB)'
  );
};
/**
...
const enableReminder = (message, client) => {
  ...
  database.serialize(() => {
     ...
     */
    database.get(
      `SELECT * FROM Reminder WHERE guild_id = ${message.guild.id} AND enabled = ${true}`,
      (error, enabledReminder) => {
        if (error) {
          console.log(error);
        }
        ...
        if (enabledReminder) {
          ...
          database.get(
            `SELECT * FROM Reminder WHERE guild_id = ${message.guild.id} AND channel_id = ${message.channel.id}`,
            (error, reminder) => {
              if (error) {
                console.log(error);
              }
              ...
              if (reminder) {
                ...
                database.serialize(async () => {
                  ...
                  database.run(
                    `UPDATE Reminder SET enabled = ${true}, enabled_by = "${
                      message.author.id
                    }", enabled_at = ${Date.now()} WHERE uuid = "${reminder.uuid}"`,
                    (err) => {
                      if (err) {
                        console.log(err);
                      }
                  ...
                  database.get(
                    `SELECT * FROM Role WHERE guild_id = "${
                      message.guild.id
                    }" AND used_for_reminder = ${true}`,
                    async (err, role) => {
                      if (err) {
                        console.log(err);
                      }
                      if (role) {
                        ...
                        database.run(
                          `UPDATE Reminder SET role_uuid = "${role.uuid}" WHERE uuid = "${reminder.uuid}"`,
                          (error) => {
...
const insertNewReminder = (message, client) => {
  ...
  database.serialize(() => {
     ...
     */
    database.run(
      'INSERT INTO Reminder(uuid, created_at, enabled, enabled_by, enabled_at, disabled_by, disabled_at, type, role_uuid, channel_id, guild_id, reaction_message_id, timer) VALUES ($uuid, $created_at, $enabled, $enabled_by, $enabled_at, $disabled_by, $disabled_at, $type, $role_uuid, $channel_id, $guild_id, $reaction_message_id, $timer)',
      {
        $uuid: reminderUUID,
        $created_at: new Date(),
        $enabled: true,
     ...
     */
    database.get(
      `SELECT * FROM Role WHERE guild_id = ${message.guild.id} AND used_for_reminder = ${true}`,
      (error, role) => {
        if (error) {
          console.log(error);
        }
        if (role) {
          database.run(
            `UPDATE Reminder SET role_uuid = "${role.uuid}" WHERE uuid = "${reminderUUID}"`,
            (error) => {
              if (error) {
                console.log(error);
              }
              database.get(
                `SELECT * FROM Reminder WHERE uuid = "${reminderUUID}"`,
                (error, reminder) => {
                  if (error) {
                    console.log(error);
                  }
      ...
      (error, reminder) => {
        ...
        if (reminder) {
          ...
          if (reminder.enabled === 0) {
            ...
            database.run(
              `UPDATE Reminder SET enabled = ${false}, disabled_by = "${
                message.author.id
              }", disabled_at = ${Date.now()} WHERE guild_id = ${
                message.guild.id
              } AND channel_id = ${message.channel.id}`,
              (err) => {
                if (err) {
                  console.log(err);
                }
            ...
            database.get(
              `SELECT * FROM Role WHERE uuid = "${reminder.role_uuid}"`,
              (error, role) => {
                database.get(
                  `SELECT * FROM ReminderReactionMessage WHERE guild_id = "${message.guild.id}"`,
                  async (error, reactionMessage) => {
                    if (reactionMessage) {
                      const reactionChannel = await client.channels.fetch(
                        reactionMessage.channel_id
                      );
                      const reactionMessageInChannel = await reactionChannel.messages.fetch(
                        reactionMessage.uuid
                      );
                      const updatedReactionMessage = reminderReactionMessage(
                        null,
                        role && role.role_id
                      );
                      await reactionMessageInChannel.edit(
                        new Discord.MessageEmbed(updatedReactionMessage)
                      ); //Creates new discord embed and pass it in when editing the reaction message
                    }
                  }
                );
              }
...
const createReminderRole = async (message, reminder, client) => {
  ...
  try {
    const reminderRole = await message.guild.roles.create({
      ...
      reason:
        'Role to be used by Yagi for automated reminders for Vulture Vale/Blizzard Berg World Boss',
    });
    database.serialize(() => {
      database.get(
        `SELECT * FROM Role WHERE role_id = ${reminderRole.id} AND guild_id = ${reminderRole.guild.id}`,
        (error, role) => {
          if (error) {
            console.log(error);
          }
          if (role) {
            ...
            database.run(
              `UPDATE Role SET reminder_id = "${
                reminder.uuid
              }", used_for_reminder = ${true} WHERE uuid = "${role.uuid}"`,
              (err) => {
                if (err) {
                  console.log(err);
                }
...
const sendReminderInformation = (message, yagi) => {
  ...
  database.get(
    ...
    (error, enabledReminder) => {
      ...
      if (enabledReminder) {
        database.get(
          `SELECT * FROM Role WHERE uuid = "${enabledReminder.role_uuid}"`,
          (error, role) => {
            if (error) {
              console.log(error);
            }
            database.get(
              `SELECT * FROM ReminderReactionMessage WHERE uuid = "${enabledReminder.reaction_message_id}"`,
              async (error, reactionMessage) => {
                if (error) {
                  console.log(error);
                }
                ...
                if (reactionMessage) {
                  ...
                  const reactionMessageInChannel = await reactionChannel.messages.fetch(
                    reactionMessage.uuid
                  ); //Fetches message data from discord
                  const embed = reminderDetails(
                    enabledReminder.channel_id,
                    role && role.role_id,
                    reactionMessageInChannel.url
                  );
                  message.channel.send({ embeds: [embed] });
                } else {
                  const embed = reminderDetails(
  ...
  database.get(`SELECT * FROM Timer WHERE rowid = ${1}`, (error, timer) => {
    ...
    if (timerCountdown >= 601000 && !isInWeeklyMaintenance(timer.accurate === 1)) {
      ...
      const reminderTimeout = setTimeout(async () => {
        const reminderTimerMessage = await sendReminderTimerEmbed(
          reminderChannel,
          role && role.role_id,
          timer
        );
      ...
      database.run(
        `UPDATE Reminder SET timer = ${reminderTimeout} WHERE uuid = "${reminder.uuid}"`,
        (error) => {
          if (error) {
            console.log(error);
          }
